...
public class Main {
    ...
    // 测试当面付2.0查询订单
    public String test_trade_query(String orderId) {
...
*/
    ...
    // 测试当面付2.0生成支付二维码(返回)
    public OrderSo test_trade_precreate(Order order) {
    	//生产并二维码的保存地址
    	 String uuid = UUID.randomUUID().toString()
  				.replace("-", "");
          int randomNum = new Random().nextInt(1000);
          String type=".png";
         String name =uuid + randomNum +type;
         String dateDir = 
      			new SimpleDateFormat("yyyy/MM/dd").format(new Date());
      			
      			//3.1判断文件夹是否存在 e:jt-upload/yyyy/MM/dd
      	 
      			String dirFilePath = "E:/jt-upload" + "/" + dateDir;
      			...
      			//String url =localUrlPath+"/" + dateDir + "/" + name;
          String url1 ="E:/jt-upload"+"\\" + dateDir + "\\" + name;
        String url =localUrlPath+"\\" + dateDir+"\\"+name;
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
      /*  String outTradeNo = "tradeprecreate" + System.currentTimeMillis()
        ...
        List<GoodsDetail> goodsDetailList = new ArrayList<GoodsDetail>();
        // 创建一个商品信息，参数含义分别为商品id（使用国标）、名称、单价（单位为分）、数量，如果需要添加商品类别，详见GoodsDetail
        GoodsDetail goods1 = GoodsDetail.newInstance("goods_id002", order.getName(), order.getPrice(), 1);
        // 创建好一个商品后添加至商品明细列表
        goodsDetailList.add(goods1);
     ...
     /*   GoodsDetail goods2 = GoodsDetail.newInstance("goods_id002", "xxx牙刷", 500, 2);
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format(url1,
        ...
        orderSo.setUrl(url);
        orderSo.setOrderId(order.getId());
        return orderSo;
    }
}
