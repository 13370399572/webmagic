...
$('.register-tab li').click(function() {
	...
	$('.register-wp .register-box').hide().eq(lid).show().find('.php-code').click();
});
...
function registerAccount(){
	var param = {
		...
		'code' : $('.register-account input[name=code]').val(),
		'is_ajax' : 1
	}
	$.post('/register.php', param, function(result) {
		if (result.msg == '注册成功' || result.msg == '登录成功') {
			var vurl = $('.register-account input[name=gourl]').val(), vurl = vurl ? vurl : C9377.url;
...
function sendCaptcha(btn){
	var checkU = $('#phone').siblings('b').html();
	if (checkU.indexOf('已存在') != -1) {
		return false;
	}
	var phone = $('#phone'), vphone = phone.val(), ptip = phone.siblings('b'),
		code = $('#pcode'), vcode = code.val(), ctip = code.siblings('b'), 
		vbtn = $(btn);
	if (vphone.length < 11) {
...
function registerPhoneAccount(){
	var param = {
		...
		'NAME' : $('.register-phone input[name=xNAME]').val(),
		'ID_CARD_NUMBER' : $('.register-phone input[name=xID_CARD_NUMBER]').val(),
		'is_ajax' : 1
	}
	$.post('/register.php', param, function(result) {
		if (result.msg == '注册成功' || result.msg == '登录成功') {
			var vurl = $('.register-phone input[name=gourl]').val(), vurl = vurl ? vurl : C9377.url;
