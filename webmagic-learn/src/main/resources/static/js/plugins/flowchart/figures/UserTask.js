...
/*
com.mvr.unfetter.domain.workflow.UserTaskICON = draw2d.shape.icon.User.extend({
	NAME: "UserTaskICON",
...
com.mvr.unfetter.domain.workflow.UserTaskICON = draw2d.SetFigure.extend({
	NAME: "UserTaskICON",
...
com.mvr.unfetter.domain.workflow.UserTaskText = draw2d.shape.basic.Text.extend({
	NAME: "UserTaskText",
...
com.mvr.unfetter.domain.workflow.UserTask = draw2d.shape.basic.Rectangle.extend({
	NAME: "com.mvr.unfetter.domain.workflow.UserTask",
	init: function(ele){
		var textName = "工序";
		var userData;
		var nodeJson;
		if(ele){
			nodeJson = JSON.parse(ele);
			textName = nodeJson.userData.name;
		...
		var userTaskICON = new com.mvr.unfetter.domain.workflow.UserTaskICON();
		
		var userTaskText = new com.mvr.unfetter.domain.workflow.UserTaskText(textName);
		if(ele){
			userTaskText.id = nodeJson.id;
		}
...
//                "blue":   {name: "复制", icon: "copy"},
	...
	openPropertiesPanel: function(){//15867
//		eastPanel.panel('refresh','properties/userTaskProperties.html');
	},
	deleteRelationship: function(){
		var id = this.id;
		//删除流程和物料、人员关系
		ajaxDelete(basePath + "/flowChartMaterial/deleteRelationship?stepId"+id, null, function(data){
            if(data.success){
                modals.correct("已删除该数据");
            }else{
                modals.error("数据被引用，不可删除！");
            }
        });
	},
